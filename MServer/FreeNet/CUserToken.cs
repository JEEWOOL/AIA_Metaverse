...
namespace FreeNet
{
	public class CUserToken
	{
		...
		{
			/* TCP는 정확한 전송을 위해서 흐름제어를 한다.
			 * 흐름제어는 통신망의 상황에 따라 패킷의 크기를 유동적으로 송수신하는 것이다.
			 * 그러므로 Application계층에서 보낸 패킷이 
			 * 상황에 따라 분리되거나 합쳐져서 가게 된다.
			 * 그러므로 정확한 처리를 위해서는 프로토콜 설계에 따른
			 * 패킷의 구분이 필요하다.
			 프로토콜 설계에 따라 정확하게 패킷을 만들어주는 역할

			 * on_message : 프로토콜에 따른 패킷1개가 완성되면 호출할 콜백 함수
			*/
