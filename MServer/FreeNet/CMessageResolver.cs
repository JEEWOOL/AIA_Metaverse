...
namespace FreeNet
{
	...
	class CMessageResolver
	{
		...
		bool read_until(byte[] buffer, ref int src_position, int offset, int transffered)
		{
			...
			// 버퍼에 복사.
			//SocketAsyncEventArgs객체의 buffer -> this.message_buffer로 데이터를 복사한다
			...
			// 목표지점에 도달 못했으면 false(Header or Body)
		...
		public void on_receive(byte[] buffer, int offset, int transffered, CompletedMessageCallback callback)
		{
			...
			while (this.remain_bytes > 0)
			{
				...
				if (this.current_position < Defines.HEADERSIZE)
				{
					...
					/* completed
					 *   - true : 헤더를 다 읽었다.
					 *   - false : 헤더크기만큼 다 읽지 못했다(데이터가 아직 더 수신 되었다)
					 */
                ...
                /* completed
				 *   - true : Body를 다 읽었다.
				 *   - false : Body크기만큼 다 읽지 못했다(데이터가 아직 더 수신 되었다)
				 */
                if (completed)
				{
                    /*-----------------Body를 읽는 영역 end-------------------------*/
                    ...
                    /*-----------------패킷 처리 영역 start-------------------------*/
                    // 완성된 패킷을 처리하기 위해 등록된 콜백 메서드를 호출
                    // 패킷 하나를 완성 했다.
                    byte[] tempArr = new byte[1024];
                    Array.Copy(this.message_buffer, tempArr, this.position_to_read);
                    Task.Run(() =>
					{
                        callback(new Const<byte[]>(tempArr));
					});

					// this.message_buffer를 다음 패킷완성에 사용해야 하므로
					// 깨끗하게 비우고 초기화 한다.
					clear_buffer();
                    /*-----------------패킷 처리 영역 end-------------------------*/
                }
